...
class Program
  ...
  def check_for_reachable_stop
    ...
    @lines.each do |line_number, line|
      ...
      statements.each_with_index do |statement, stmt|
        found_stop = true if
          statement.reachable && (statement.stop? || statement.end?)
      end
    end
    ...
    unless found_stop
      lines << ''
      lines << 'No path from start to STOP or END.'
    end
  ...
  def find_closing_next_line_stmt(control_variable, current_line_stmt)
    ...
    until walk_line_stmt.nil?
      ...
      if statement.next?
        stmt_control = statement.control_variable

        for_level -= 1

        break if for_level.negative?

        if stmt_control == control_variable ||
