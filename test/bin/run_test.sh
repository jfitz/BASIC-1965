...
then
    ...
    then
	cd "$TESTBED/$TESTNAME"
	ruby basic.rb --no-timing $OPTIONS --run $TESTNAME.bas --print-width 0 --no-heading --trace <stdin.txt >trace.txt $GROUP_OPTIONS $TEST_OPTIONS $GROUP_RUN_OPTIONS $RUN_OPTIONS
	cd ../..
    else
	cd "$TESTBED/$TESTNAME"
