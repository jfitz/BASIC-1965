...
|==========
|Function |Result
|ABS(x) |numeric scalar |Computes the absolute value of x.
|ARCCOS(x) |numeric scalar |Computes the arccosine of x, providing the answer in radians.
|ARCSIN(x) |numeric scalar |Computes the arcsine of x, providing the answer in radians.
|ARCTAN(x) |numeric scalar |Computes the arctangent of x, providing the answer in radians.
|ARCTAN(y,x) |numeric scalar |Computes the arctangent of y and x, providing the answer in radians.
|ATN(x) |numeric scalar |Synonym for ARCTAN(x).
|ATN(y,x) |numeric scalar |Synonym for ARCTAN(y,x).
...
|COS(x) |numeric scalar |Computes the cosine of x, where x is in radians.
|COT(x) |numeric scalar |Computes the cotangent of x, where x is in radians.
|CSC(x) |numeric scalar |Computes the cosecant of x, where x is in radians.
|DET(m)	|numeric scalar |Computes the determinant of the matrix. The matrix must be square.
|EXP(x) |numeric scalar |Computes e to the x power.
|FIX(x) |numeric scalar |Returns the integer part of a number. Truncates towards negative infinity.
...
|INT(x) |numeric scalar |Return the integer part of a numeric value. Truncates towards zero.
|INV(m) |numeric matrix |Inverts matrix m.
|LOG(x) |numeric scalar |Computes the natural log of x. A value of zero or a negative value will cause an error.
|LOG10(x) |numeric scalar |Computes the base-10 log of x. A value of zero or a negative value will cause an error.
|LOG2(x) |numeric scalar |Computes the base-2 log of x. A value of zero or a negative value will cause an error.
|MOD(a,b) |numeric scalar |Computes 'a modulus b'.
|NELEM(a) |numeric scalar |Number of elements in array a.
|RND(x) |numeric scalar |Return a random number from zero to x. The value is a floating-point value; RND(1) can return any value from zero to 1. A value of zero or less than zero is considered to be 1.
|RND() |numeric scalar |Equivalent to RND(1).
|RND |numeric scalar |Special form of RND, equivalent to RND(1).
|ROUND(x,p) |numeric scalar |Rounds the value x to p places.
|SEC(x) |numeric scalar |Computes the secant of x, where x is in radians.
|SGN(x) |numeric scalar |Returns 1, 0, or -1, for positive, zero, or negative values of x.
|SIN(x) |numeric scalar |Computes the sine of x, where x is in radians.
|SQR(x) |numeric scalar |Computes the square root of x. A negative value will cause an error.
|TAN(x) |numeric scalar |Computes the tangent of x, where x is in radians.
